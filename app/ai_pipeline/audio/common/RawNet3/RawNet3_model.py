# Vendored from Jungjee/RawNet, python/RawNet3 (MIT License) via heimdall-vox/RawNet3.
# See LICENSE/NOTICE in this directory. models/RawNet3.py + models/RawNetBasicBlock.py
# merged into this single file (originally two files, unmodified content).
#
# Upstream RawNet3 is a VoxCeleb speaker-verification model: RawNet3.forward() returns
# an nOut-dim speaker embedding, not a real/fake score. RawNet3Classifier at the bottom
# of this file (project-original, not vendored) wraps the backbone with a 1-neuron
# classification head to adapt it for the real/fake binary task.
import math
...
class RawNet3Classifier(nn.Module):
    """RawNet3 화자검증 백본 + real/fake 1-neuron 분류 헤드 (project-original, not vendored).
    forward()는 raw logit(sigmoid 이전)을 반환한다."""

    def __init__(self, embedding_dim=256):
        super().__init__()
        self.backbone = MainModel(
            encoder_type="ECA",
            nOut=embedding_dim,
            out_bn=False,
            sinc_stride=10,
