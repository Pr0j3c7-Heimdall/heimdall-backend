...
class AudioHistoryItem(BaseModel):
    history_id: int = Field(..., description="분석 요약 테이블(audio_final_detection_results)의 PK")
...
class AudioHistoryData(BaseModel):
    total_count: int = Field(..., description="조건에 맞는 전체 데이터 개수")
    total_pages: int = Field(..., description="전체 페이지 수")
    current_page: int = Field(..., description="현재 페이지 번호")
    histories: List[AudioHistoryItem] = Field(..., description="검증 내역 목록")

class AudioHistoryResponse(SuccessResponse):
    data: AudioHistoryData
